...
# Minor updates to better differentiate multiple switches on a singel device by Justion Gasper, 2023
...
class HomeBridgeController:
    ...
    def _get_accessories(self) -> bool:
        ...
        for accessory in get_response.json()['accessories']:
            devices = self._get_info_of_accessory(accessory)
    ...
    def _get_info_of_accessory(accessory_dict: Dict) -> Tuple[str, Dict]:
        ...
        for service_info in accessory_dict['services']:
            ...
            if a_value is not None:
                ...
                a_format = a_value[2]
                a_value = a_value[1]
            devices.append((a_name, {'aid': a_id, 'iid': a_iid, 'type': a_type, 'value': a_value, 'format': a_format}))
        return devices
